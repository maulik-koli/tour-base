import { useQuery, UseQueryOptions, UseQueryResult } from "@tanstack/react-query";
import { safeAxios } from "@/lib/api/axios";
import { getActivities, getActivity } from "./api";

import { GetActivitiesResponse, GetActivitiesParams, GetActivityResponse, GetActivityParam } from "./types";
import { ApiError, ApiResponse } from "@/types/api";
import { QUERY_REGISTRY } from "@/constants/apiRegistery";


export const useGetActivities = (
    params?: GetActivitiesParams,
    options?: UseQueryOptions<
        ApiResponse<GetActivitiesResponse>,
        ApiError,
        ApiResponse<GetActivitiesResponse>
    >,
): UseQueryResult<ApiResponse<GetActivitiesResponse>, ApiError> => {

    return useQuery({
        queryKey: [QUERY_REGISTRY.getActivities, params],
        queryFn: () => safeAxios(() => getActivities(params)),
        retry: false,
        ...options,
    });
};


export const useGetActivity = (
    params: GetActivityParam,
    enabled: boolean = true,
    options?: UseQueryOptions<
        ApiResponse<GetActivityResponse>,
        ApiError,
        ApiResponse<GetActivityResponse>
    >,
): UseQueryResult<ApiResponse<GetActivityResponse>, ApiError> => {

    return useQuery({
        queryKey: [QUERY_REGISTRY.getActivities, params],
        queryFn: () => safeAxios(() => getActivity(params)),
        retry: false,
        enabled: enabled,
        ...options,
    });
};
