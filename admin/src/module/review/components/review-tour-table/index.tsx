"use client"
import React from 'react'
import { useRouter } from 'next/navigation'
import { TourReviewListItem } from '@module/review/api/types'

import Icon from '@/components/icons'
import { Button } from '@ui/button'
import { Card, CardContent } from '@ui/card'
import { 
    Table, 
    TableBody, 
    TableCell, 
    TableHead, 
    TableHeader, 
    TableRow 
} from "@/components/ui/table"

interface ReviewTourTableProps {
    tourList: TourReviewListItem[]
    pageOffset?: number
}


const ReviewTourTable: React.FC<ReviewTourTableProps> = ({ tourList, pageOffset = 0 }) => {
    const router = useRouter()

    const handleViewMore = (tourId: string) => {
        router.push(`/reviews/${tourId}`)
    }

    return (
        <Card className='w-full'>
            <CardContent className='px-0'>
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead className='pl-6 w-20'>Sr No.</TableHead>
                            <TableHead>Tour Name</TableHead>
                            <TableHead className='w-32 text-center'>Review Count</TableHead>
                            <TableHead className='pr-6 w-32 text-center'>Action</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {tourList.map((tour, index) => (
                            <TableRow key={tour._id}>
                                <TableCell className='pl-6 font-medium'>
                                    {pageOffset + index + 1}
                                </TableCell>
                                <TableCell className='font-medium'>
                                    {tour.name}
                                </TableCell>
                                <TableCell className='text-center'>
                                    <span className='inline-flex items-center justify-center bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-medium'>
                                        {tour.reviewCount}
                                    </span>
                                </TableCell>
                                <TableCell className='pr-6 text-center'>
                                    <Button 
                                        variant="outline"
                                        size="sm"
                                        type='button'
                                        onClick={() => handleViewMore(tour._id)}
                                    >
                                        <Icon name="Eye" width={16} height={16} />
                                        View More
                                    </Button>
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            </CardContent>
        </Card>
    )
}

export default ReviewTourTable
